...
function updateDocumentsList() {
  ...
  for(var i = 0; i < entries.length; ++i) {
   ...
   '</svg>'+
     ...
     '<p class="text-xs text-gray-900 dark:text-white">'+ doc.description +'</p>'+
...
function updateTagsList() {
  ...
  for(var i = 0; i < entries.length; ++i) {
    ...
    elem.innerHTML =
     ...
     ' <span class=" text-blue-700 text-xs font-medium inline-flex rounded-lg items-center px-2.5 py-0.5 ">'+
     '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">'+
     '<path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" />'+
     '<path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z" />'+
   '</svg>'+
   '<a href="javascript:editTag(' + tag.id + ');" class="px-3 py-2 text-xs text-left inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" >' 
   + gettext("Edit Tag") + '</a>' +
   '</span>'+
